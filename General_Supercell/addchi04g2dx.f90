!=========================================================================
!This program reads in polarizability of two layer system from chi0mat1 
!and chi0mat2 generated by BGW epsilon. 
!Calculate the total polarizability and write into file chi0mat
!                  
!=========================================================================

program chi0simpleadd
...
!  integer :: x, y, z, xp, yp, zp
  ...
  integer :: freq_dep, nFreq, nband, nrk, icount, nspin
...
!------------program start-----------------
  open(14,file='chi0mat_bnG',form='unformatted',status='old')
  open(15,file='chi0mat_add',form='unformatted',status='replace')
  open(16,file='diagplot0',form='formatted',status='replace')
  open(17,file='offdiagplot0',form='formatted',status='replace')
...
! Read chi0mat_G    sys_2
      if (initial_read) then
        ...
        read(14)
        read(14)
        open(12,file='chi0mat_G',form='unformatted',status='old',iostat=ioerr)
        read(12)
        read(12)
        read(12)
        read(12)
...
!      read(12) nmtx2,np,(isrtx2(i),gvec2%ekin(i),i=1,gvec2%ng)
      ...
      if(nspin .eq. 2) then
      allocate (spchimat2_ggp (nmtx2,nmtx2), stat=allocerr)
      allocate (spchimat3_ggp (nmtx2,nmtx2), stat=allocerr)
      endif
      do igp = 1,nmtx2
      ...
      do igp = 1,nmtx2
        ! read the chimat2 matrix column-by-column
        read(12) (spchimat2_ggp(ig,igp),ig=1,nmtx2)
        read(14) (spchimat3_ggp(ig,igp),ig=1,nmtx2)
      enddo
      endif
!-----------------------------
! Read Chimat1. Add chimat2 and write
      ...
      if (initial_read) then
        open(11,file='chi0mat_bn',form='unformatted',status='old',iostat=ioerr)
...
!      write(15) nmtx1,np,(isrtx1(i),gvec1%ekin(i),i=1,gvec1%ng),(irow(i),i=1,nmtx1)
      ...
      do igp = 1,nmtx1
        ...
        do ig = 1,nmtx1
          spchimat_ggp(ig,igp)=spchimat1_ggp(ig,igp)+spchimat2_ggp(ig,igp)
        enddo
        ! write the chimat matrix column-by-column
...
!        write(17,'(i4,4f14.9)') igp,temp1,temp2,temp3,temp4
       enddo
      enddo
        write(*,*) 'icount,max error',icount,temp4
    deallocate (isrtx1)
    deallocate (isrtx2)
    deallocate (irow)
...
!  write(*,*) "chimat_M",chimat1_ggp(1,1),chimat1_ggp(nmtx,nmtx)
  ...
  close(16)
  close(17)
  write(*,*) "Done reading and writing chi0mat"
!----------------------------------
  write(*,*) "Finish"

end program chi0simpleadd
